...
def subprocess_fn(rank, args, temp_dir):
    ...
    DDIM = copy.deepcopy(args.DDIM).eval().requires_grad_(False).to(device)
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, snapshot_pkl=args.network_pkl, diffusion=diffusion, ddim=DDIM)
        if rank == 0:
...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    with dnnlib.util.open_url(network_pkl, verbose=args.verbose) as f:
        ...
        args.DDIM = network_dict['DDIM_ema'] # subclass of torch.nn.Module
        # args.DDIM_ema = network_dict['DDIM_ema'] # subclass of torch.nn.Module
