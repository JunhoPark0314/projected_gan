...
import dnnlib
from training import ddim_training_loop as training_loop
...
@click.option('--cfg',          help='Base configuration',                                      type=click.Choice(['ncsnpp', 'ncsnpp_fast', 'ddim']), required=True)
@click.option('--g_cfg',        help='path to ddim_latent config',                                      type=str, required=True)
@click.option('--ddim_config',  help='path to ddim config',                                      type=str, required=True)
@click.option('--ddim_args',    help='path to ddim args',                                      type=str, required=True)
...
def main(**kwargs):
    ...
    if opts.cfg == 'ncsnpp':
        ...
        c.G_kwargs.class_name = 'pg_modules.score_sde.models.ncsnpp_generator_adagn.NCSNpp'
        with open(opts.g_cfg, "r") as stream:
            c.G_kwargs.config = dict2namespace(yaml.safe_load(stream))
        use_separable_discs = True
    elif opts.cfg == 'ncsnpp_fast':
        c.G_kwargs = dnnlib.EasyDict(class_name='pg_modules.networks_fastgan_ddpm.Generator', cond=opts.cond, synthesis_kwargs=dnnlib.EasyDict())
        c.G_kwargs.synthesis_kwargs.lite = (opts.cfg == 'fastgan_lite')
        c.G_kwargs.ngf = 64
        c.G_opt_kwargs.lr = c.D_opt_kwargs.lr = 0.0002
        with open(opts.g_cfg, "r") as stream:
            c.G_kwargs.ddim_config = dict2namespace(yaml.safe_load(stream))
        use_separable_discs = True
    elif opts.cfg == 'ddim':
        c.G_kwargs = dnnlib.EasyDict(class_name='ddim.models.diffusion.Model')
        with open(opts.g_cfg, "r") as stream:
            c.G_kwargs.config = dict2namespace(yaml.safe_load(stream))
    ...
    with open(opts.ddim_config, "r") as stream:
        c.ddim_config = dict2namespace(yaml.safe_load(stream))
    with open(opts.ddim_args, "r") as stream:
        c.ddim_args = dict2namespace(yaml.safe_load(stream))
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.ProjectedPairedGANLoss')
    c.D_kwargs = dnnlib.EasyDict(
        class_name='pg_modules.discriminator.ProjectedPairedDiscriminator',
        diffaug=False,
        interp224=False,
