...
import dnnlib
from training import training_loop_ddim as training_loop
...
@click.option('--cfg',          help='Base configuration',                                      type=click.Choice(['fastgan', 'fastgan_lite', 'stylegan2', 'fastgan_enc_lite', 'fastgan_enc']), required=True)
...
def main(**kwargs):
    ...
    elif opts.cfg in ['fastgan_enc', 'fastgan_enc_lite']:
        c.G_kwargs = dnnlib.EasyDict(class_name='pg_modules.networks_fastgan_enc.Generator', cond=opts.cond, synthesis_kwargs=dnnlib.EasyDict())
        c.G_kwargs.synthesis_kwargs.lite = (opts.cfg == 'fastgan_enc_lite')
        c.G_opt_kwargs.lr = c.D_opt_kwargs.lr = 0.0002
        use_separable_discs = False
    ...
    # Description string.
    desc = f'{opts.cfg:s}-ddim-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}'
    if opts.desc is not None:
        desc += f'-{opts.desc}'

    # Projected and Multi-Scale Discriminators
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.ProjectedGANPairLoss')
    c.D_kwargs = dnnlib.EasyDict(
        class_name='pg_modules.discriminator.ProjectedPairDiscriminator',
        diffaug=True,
