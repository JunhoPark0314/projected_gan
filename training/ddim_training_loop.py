...
def training_loop(
		...
		metric_ticks = network_snapshot_ticks * 20
		# Initialize.
		...
		# netTrg = Denoiser(ddim_config)
		netTrg.load_state_dict(torch.load(ddim_args.pretrained))
		# G.load_state_dict(torch.load(ddim_args.pretrained), strict=False)
		netTrg.eval().requires_grad_(False).to(device)
		diffusion = Diffusion(ddim_args, ddim_config, device)
		...
		common_kwargs = dnnlib.EasyDict(common_kwargs)

		# Print network summary tables.
		if rank == 0:
				t = torch.ones([batch_gpu,], device=device) * 999
				x_init = torch.randn((batch_gpu, common_kwargs.img_channels, common_kwargs.img_resolution, common_kwargs.img_resolution), device=device)
				eps = netTrg(x_init, t)
				x_init = diffusion.denoise_step(x_init, eps, t)

				z = torch.empty([batch_gpu, G.z_dim], device=device)
				# c = torch.empty([batch_gpu, G.c_dim], device=device)
				img = misc.print_module_summary(G, [x_init, t, z])
				misc.print_module_summary(D, [img, t])

		# Distribute across GPUs.
		if rank == 0:
				print(f'Distributing across {num_gpus} GPUs...')
		for module in [G, D, G_ema, netTrg]:
		...
		loss = dnnlib.util.construct_class_by_name(device=device, G=G, G_ema=G_ema, D=D, netTrg=netTrg, diffusion=diffusion, **loss_kwargs) # subclass of training.loss.Loss
		phases = []
		...
		if rank == 0:
				...
				grid_z = torch.randn([labels.shape[0], diffusion.num_step+1, G.z_dim], device=device).split(batch_gpu)
				grid_x = torch.randn([labels.shape[0], common_kwargs.img_channels, common_kwargs.img_resolution, common_kwargs.img_resolution], device=device).split(batch_gpu)

				images = torch.cat([diffusion.sample_image(G_ema, netTrg, x_init, z).cpu() for z, x_init in zip(grid_z, grid_x)]).numpy()
				save_image_grid(images[-1], os.path.join(run_dir, 'fakes_init.png'), drange=[-1,1], grid_size=grid_size)
				save_image(make_grid((torch.tensor(images[:,:len(images)*4]).flatten(0,1) + 1)*0.5), os.path.join(run_dir, 'fakes_init_seq.png'))
		...
		while True:
				...
				for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
						...
						if phase.name in ['Dmain', 'Dboth', 'Dreg']:
								...
								#phase.module.proj_network.requires_grad_(False)
				...
				if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
						images = torch.cat([diffusion.sample_image(G_ema, netTrg, x_init, z).cpu() for z, x_init in zip(grid_z, grid_x)]).numpy()
						save_image_grid(images[-1], os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}.png'), drange=[-1,1], grid_size=grid_size)
						save_image(make_grid((torch.tensor(images[:,:len(images)*4]).flatten(0,1) + 1)*0.5), os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_seq.png'))
						# diffusion.update_num_step()
						# grid_z = torch.randn([labels.shape[0], diffusion.num_step+1, G.z_dim], device=device).split(batch_gpu)
						next_t, prev_t = diffusion.sample_time(real_images)
						a_next, a_prev, next_xt, prev_xt, prev_x0, next_x0 = diffusion.sample_noised_pair(real_images, next_t, prev_t, netTrg)
						next_eps = G_ema(prev_x0, prev_t, torch.randn((prev_x0.shape[0], G_ema.z_dim), device=device))
						next_x0_gen = diffusion.gen_noised(next_eps, a_prev, a_next, prev_xt, return_x0=True)
						save_image(make_grid(torch.cat([real_images, prev_xt, prev_x0, next_xt, next_x0, next_x0_gen])+1)*0.5, 
										os.path.join(run_dir, f'shot{cur_nimg//1000:06d}.png'))
				...
				if (rank == 0) and (restart_every > 0) and (network_snapshot_ticks is not None) and (
						...
						snapshot_pkl = misc.get_ckpt_path(run_dir, cur_tick)
						base_pkl = misc.get_ckpt_path(run_dir)
				...
				if cur_tick and (snapshot_data is not None) and (len(metrics) > 0) and (cur_tick % metric_ticks == 0):
						if rank == 0:
								print('Evaluating metrics...')
						for metric in metrics:
								result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], run_dir=run_dir, cur_nimg=cur_nimg,
										dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, diffusion=diffusion, netTrg=netTrg)
								if rank == 0:
