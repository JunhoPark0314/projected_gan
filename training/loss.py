...
class ProjectedGANPairLoss(Loss):
    ...
    def run_G(self, real_img, z, c, temp_max=None, update_emas=False):
        h, enc, ws, scale = self.G.mapping(real_img, z, c, temp_max=temp_max, update_emas=update_emas)
        high, low, _ = self.G.synthesis(h, ws, c, scale, update_emas=False)
        return high, low, scale.squeeze()[:,None], h, enc
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        if do_Gmain:
            ...
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                loss_Gmain = (-gen_logits).mean()
                loss_rec = ((h_proj - enc).square() * F.relu((-gen_logits).mean(-1, keepdims=True))).mean()
        ...
        if do_Dmain:
            ...
            # Dmain: Minimize logits for generated images.
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img_high, gen_img_low, scale, h_proj, _ = self.run_G(real_img, gen_z, gen_c, temp_max=warmup, update_emas=True)
                gen_logits = self.run_D(gen_img_high, h_proj, gen_c, scale, blur_sigma=blur_sigma)
                loss_Dgen = (F.relu(torch.ones_like(gen_logits) + gen_logits)).mean()
                # gen_pair_logits = self.run_E(gen_img_low, h_proj, scale)
