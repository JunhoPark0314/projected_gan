...
def training_loop(
    ...
    batch_gen = 4
    evaluation_set_sampler = misc.InfiniteSampler(dataset=training_set, rank=rank, num_replicas=num_gpus, seed=random_seed)
    evaluation_set_iterator = iter(torch.utils.data.DataLoader(dataset=training_set, sampler=training_set_sampler, batch_size=batch_gen, **data_loader_kwargs))
    ...
    if rank == 0:
        x = torch.empty([batch_gpu, training_set.num_channels, training_set.resolution, training_set.resolution], device=device)
        z = torch.empty([batch_gpu, G_ema.z_dim], device=device)
    ...
    if rank == 0:
        ...
        grid_x = torch.randn([labels.shape[0], *DDIM.input_size]).split(16)
        grid_z = torch.randn([labels.shape[0], G_ema.z_dim], device=device).split(16)
        grid_c = torch.from_numpy(labels).to(device).split(16)

        images = []
        for i, (x, z, c) in enumerate(zip(grid_x, grid_z, grid_c)):
            h_sample = diffusion.sample(DDIM_ema, x.to(device))
            h_noised, scale, alpha = G_ema.mapping.sample_noised(h_sample.to(device), temp_min=i/len(grid_x), temp_max=(i+1)/len(grid_x))
            high, _, _ = G_ema.synthesis(h=h_noised, input=z.unsqueeze(1), c=c, scale=scale, alpha=alpha)
            images.append(high.cpu())
    ...
    while True:
        ...
        for phase in phases:
            ...
            for real_img in phase_real_img:
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, cur_nimg=cur_nimg)
        ...
        # Update G_ema.
        with torch.autograd.profiler.record_function('DDIM_ema'):
            ema_nimg = ema_kimg * 1000
            if ema_rampup is not None:
                ema_nimg = min(ema_nimg, cur_nimg * ema_rampup)
            ema_beta = 0.5 ** (batch_size / max(ema_nimg, 1e-8))
            for p_ema, p in zip(DDIM_ema.parameters(), DDIM.parameters()):
                p_ema.copy_(p.lerp(p_ema, ema_beta))
            for b_ema, b in zip(DDIM_ema.buffers(), DDIM.buffers()):
                b_ema.copy_(b)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            for i, (x, z, c) in enumerate(zip(grid_x, grid_z, grid_c)):
                ...
                # high = G_ema(x=x.to(device), z=z, c=c, temp_min=i/len(grid_x), temp_max=(i+1)/len(grid_x), noise_mode='const')
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], run_dir=run_dir, cur_nimg=cur_nimg,
                    dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, diffusion=diffusion, ddim=DDIM_ema)
                if rank == 0:
