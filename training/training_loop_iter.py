...
def training_loop(
    ...
    if rank == 0:
        ...
        proj = torch.randn((len(img),G.mapping.out_ch, 16,16), device=img.device)
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], run_dir=run_dir, cur_nimg=cur_nimg,
                    dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, data_iter=evaluation_set_iterator)
                if rank == 0:
